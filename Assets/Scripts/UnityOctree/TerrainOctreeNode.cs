...
public class TerrainOctreeNode {
	...
	public void GetNearby(ref Ray ray, int maxDistance, List result) {
	...
	/// <param name="result">List result.</param>
	/// <returns>Objects within range.</returns>
	public void GetNearby(ref Vector3Int position, int maxDistance, List result) {
	...
	public TerrainOctreeNode ShrinkIfPossible(int minLength) {
	...
	*/
	...
	void Merge() {
		...
		for (int i = 0; i < 8; i++) {
			TerrainOctreeNode curChild = children[i];
	...
	bool ShouldMerge() {
		...
		if (children != null) {
			foreach (TerrainOctreeNode child in children) {
