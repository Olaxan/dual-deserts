...
public class PointOctreeInt<T> {
	// The total amount of objects currently in the tree
	public int Count { get; private set; }

	// Root node of the octree
	PointOctreeIntNode<T> rootNode;
	...
	public PointOctreeInt(int initialWorldSize, Vector3Int initialWorldPos, int minNodeSize) {
		...
		minSize = minNodeSize;
		rootNode = new PointOctreeIntNode<T>(initialSize, minSize, initialWorldPos);
	}
	...
	public void Add(T obj, Vector3Int objPos) {
	...
	public bool GetNearbyNonAlloc(Ray ray, int maxDistance, List<T> nearBy) {
		nearBy.Clear();
	...
	/// <param name="nearBy">Pre-initialized list to populate</param>
	/// <returns>True if items are found, false if not</returns>
	public bool GetNearbyNonAlloc(Vector3Int position, int maxDistance, List<T> nearBy) {
		nearBy.Clear();
	...
	public HashSet<PointOctreeIntNode<T>> GetAllLeafNodes() {
		var objects = new HashSet<PointOctreeIntNode<T>>();
	...
	void Grow(Vector3Int direction) {
		...
		PointOctreeIntNode<T> oldRoot = rootNode;
		...
		rootNode = new PointOctreeIntNode<T>(newLength, minSize, newCenter);

        if (oldRoot.HasAnyObjects()) {
            // Create 7 new octree children to go with the old root as children of the new root
            int rootPos = rootNode.BestFitChild(oldRoot.Center);
            PointOctreeIntNode<T>[] children = new PointOctreeIntNode<T>[8];
            for (int i = 0; i < 8; i++) {
                if (i == rootPos) {
                    children[i] = oldRoot;
                }
                    ...
                    children[i] = new PointOctreeIntNode<T>(oldRoot.SideLength, minSize, newCenter + new Vector3Int(xDirection * half, yDirection * half, zDirection * half));
                }
            }
