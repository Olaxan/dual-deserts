...
public class PointOctreeIntNode<T> {
	// Centre of this node
	...
	// Child nodes, if any
	PointOctreeIntNode<T>[] children = null;
	...
	public PointOctreeIntNode(int baseLengthVal, int minSizeVal, Vector3Int centerVal) {
	...
	/// <returns></returns>
	public bool Add(T obj, Vector3Int objPos) {
	...
	public void GetNearby(ref Ray ray, int maxDistance, List<T> result) {
		...
		bounds.Expand(new Vector3Int(maxDistance * 2, maxDistance * 2, maxDistance * 2));
	...
	/// <param name="result">List result.</param>
	/// <returns>Objects within range.</returns>
	public void GetNearby(ref Vector3Int position, int maxDistance, List<T> result) {
        int sqrMaxDistance = maxDistance * maxDistance;
	...
	public void GetAllLeafNodes(HashSet<PointOctreeIntNode<T>> result)
	{
		if (children == null) {
			result.Add(this);
		}
	...
	public void SetChildren(PointOctreeIntNode<T>[] childOctrees) {
	...
	public void DrawAllObjects() {
		int tintVal = SideLength / 20;
		Gizmos.color = new Color(0, 1.0f - tintVal, tintVal, 0.25f);

		foreach (OctreeObject obj in objects) {
			Gizmos.DrawSphere(obj.Pos, 0.5f);
	...
	public PointOctreeIntNode<T> ShrinkIfPossible(int minLength) {
		...
		if (children == null) {
			...
			SetValues(SideLength / 2, minSize, Vector3Int.RoundToInt(childBounds[bestFit].center));
			return this;
		}
	...
	public bool Equals(PointOctreeIntNode<T> n1, PointOctreeIntNode<T> n2)
	{
		if (n1 == null || n2 == null)
	...
	public int GetHashCode(PointOctreeIntNode<T> n1)
	{
	...
	*/
	...
	/// <param name="obj">Object to add.</param>
	/// <param name="objPos">Position of the object.</param>
	void SubAdd(T obj, Vector3Int objPos) {
	...
	/// <summary>
	/// Private counterpart to the public <see cref="Remove(T, Vector3Int)"/> method.
	/// </summary>
	/// <param name="obj">Object to remove.</param>
	/// <param name="objPos">Position of the object.</param>
	/// <returns>True if the object was removed successfully.</returns>
	bool SubRemove(T obj, Vector3Int objPos) {
		bool removed = false;
	...
	void Split() {
		...
		children = new PointOctreeIntNode<T>[8];
		children[0] = new PointOctreeIntNode<T>(newLength, minSize, Center + new Vector3Int(-quarter, quarter, -quarter));
		children[1] = new PointOctreeIntNode<T>(newLength, minSize, Center + new Vector3Int(quarter, quarter, -quarter));
		children[2] = new PointOctreeIntNode<T>(newLength, minSize, Center + new Vector3Int(-quarter, quarter, quarter));
		children[3] = new PointOctreeIntNode<T>(newLength, minSize, Center + new Vector3Int(quarter, quarter, quarter));
		children[4] = new PointOctreeIntNode<T>(newLength, minSize, Center + new Vector3Int(-quarter, -quarter, -quarter));
		children[5] = new PointOctreeIntNode<T>(newLength, minSize, Center + new Vector3Int(quarter, -quarter, -quarter));
		children[6] = new PointOctreeIntNode<T>(newLength, minSize, Center + new Vector3Int(-quarter, -quarter, quarter));
		children[7] = new PointOctreeIntNode<T>(newLength, minSize, Center + new Vector3Int(quarter, -quarter, quarter));
	}
	...
	void Merge() {
		...
		for (int i = 0; i < 8; i++) {
			PointOctreeIntNode<T> curChild = children[i];
	...
	bool ShouldMerge() {
		...
		if (children != null) {
			foreach (PointOctreeIntNode<T> child in children) {
...
public class PointOctreeIntNodeEqualityComparer<T> : IEqualityComparer<PointOctreeIntNode<T>>
{
	public bool Equals(PointOctreeIntNode<T> n1, PointOctreeIntNode<T> n2)
	{
		if (n1 == null || n2 == null)
	...
	public int GetHashCode(PointOctreeIntNode<T> obj)
	{
